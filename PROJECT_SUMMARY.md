...
| Clickable confirmation suggestions (Follow-Up Prompts) | OpenWebUI has a native "Follow-Up Prompts" feature (clickable suggestion chips after each response, generated by a separate "Task Model"). Investigated as a way to show a tappable "موافق" / "كمّل المناقشة" option instead of typing. Set the Local Task Model to a separate cheap model (away from "Arena Model", which is an unrelated model-comparison feature) — confirmed this part works. Could not find/confirm the actual toggle to enable Follow-Up Prompts display in this OpenWebUI version/instance | Would give a faster confirmation UX than typing "موافق" | Deferred — uncertain whether available in the current OpenWebUI version, or named/located differently, or gated by a server-side env var (`ENABLE_FOLLOW_UP_GENERATION`) not exposed in the UI. Needs more investigation when revisited; not blocking the core New Project Path work |
| General Manager → Team Handoff (Project Brief Gate) | Structured output (`reply`/`ready_for_team`/`project_brief`) on باجوش so he presents a summary and gets explicit confirmation before handing a complete brief (covering the whole discussion, including his own suggestions) to PM Agent — instead of PM Agent only ever seeing the user's single latest message | Discovered live: PM Agent was missing earlier context and باجوش's own opinions from the discussion; loading full chat history was rejected for cost/noise reasons | Tried live, reverted (Auto-Fix bypassed باجوش's personality) — back to a plain-text summary approach. See `NEXT_STEPS.md` Step 3b |
| File & Image Understanding for باجوش | Two Tools (same zero-extra-cost pattern as SearXNG): "Read Attached Document" (pure text extraction via `Extract From File`, no AI) and "Describe Attached Image" (a separate cheap vision model, only called when an image is attached) — باجوش's own model never changes | Discovered live: attached images/documents are currently silently dropped by the Pipe; real project requirements often come as files | Not built — deferred until the core New Project Path is solid. Needs a live test first to check what OpenWebUI actually sends for document uploads. See `NEXT_STEPS.md` Step 3c |
...
| QA / Revision loop | QA Agent checks output, Revision Agent creates correction brief on failure. Must be execution-based (real install/build/test via OpenHands), not just reading the code, specifically to catch "vibe coding" failure modes (hallucinated libraries, happy-path-only code, hidden inconsistency) — the goal is genuinely sellable, production-ready projects | Quality gate before delivery | Not built — see `NEXT_STEPS.md` Step 9 and `DECISIONS_LOG.md` |
