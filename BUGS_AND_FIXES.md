...
### 2026-06-27 — CRITICAL: all conversations shared one memory under session_id "default" since day one
- **Bug**: Every single conversation, across every test from the very first day, was being saved under the literal `session_id` value `"default"` — meaning all chats shared one combined memory. This caused widespread cross-contamination (e.g. a "shoe store" project conversation surfacing "restaurant app" content from a completely different chat), the 126-call infinite-loop incident (caused by an unrelated n8n core bug, but made far worse by `01A_Time_Context`'s `LIMIT 50` pulling from this single bloated shared history), and made any per-chat memory isolation meaningless from the start.
- **Root cause**: The OpenWebUI Pipe read `chat_id = body.get("chat_id", "default")` — but `chat_id` is **not** a key inside `body` for a basic Pipe function. Open WebUI passes it through a separate **reserved argument**, `__chat_id__`, that must be added explicitly to the `pipe()` method's signature. Since our Pipe never declared that parameter, `body.get("chat_id", ...)` always fell through to the literal fallback string `"default"` — on every single request, for the entire build.
- **Fix**: Pipe updated to v1.0.7 — `async def pipe(self, body: dict, __chat_id__: str = None) -> str`, using `chat_id = __chat_id__ or "default"`. Confirmed live: two different conversations now get two distinct, different `chat_id` values.
- **Cleanup**: Deleted all 862 contaminated rows from the live database: `DELETE FROM n8n_chat_histories WHERE session_id = 'default';` (run directly via `docker exec` on the Postgres container, using the correct credentials from `.env`: `POSTGRES_USER=n8n`, `POSTGRES_DB=n8n` — not `mkddai`/`ai_factory` as initially guessed). Also cleared all chats from the Open WebUI sidebar for a clean slate.
- **Status**: ✅ Fixed and confirmed live. **Any conversation/memory data from before this fix should be considered unreliable** — it was never properly isolated per chat.

### 2026-06-27 — n8n core bug: AI Agent node loops calling the model after a valid "stop" response
- **Bug**: While debugging the slow-response issue above, found the Agent node was calling the model (and Memory) up to 126 times for a single simple message like "ازيك" — and every single one of those calls produced a complete, valid, sensible reply. The Agent kept re-calling anyway instead of accepting the first valid answer.
- **Root cause**: This is a known bug in n8n's AI Agent node core (not anything in our configuration) — confirmed against a public n8n GitHub issue describing the exact same symptom: the agent gets a model response with `finish_reason: stop`, but the executor loops and calls the model again anyway, continuing until it hits the iteration limit. Reported as a regression introduced in recent Agent node versions. Deleting and rebuilding the node from scratch, and disconnecting all Tools, did not fix it — consistent with this being a framework-level bug rather than a configuration issue.
- **Fix (mitigation, not a root-cause fix)**: Added an explicit `Max Iterations` value (`2`) under the Agent node's Options, to cap the damage/cost if the bug recurs, since the underlying n8n bug cannot be fixed from our side. Note: most of the 126-call severity was compounded by the session_id "default" bug above (`01A_Time_Context` pulling from one giant shared history) — with that fixed, this bug's day-to-day impact should be much smaller, but the `Max Iterations` cap stays in place as a safety net.
- **Status**: ⏳ Mitigated (capped), root cause is an upstream n8n bug outside our control. Watch for an n8n update that resolves it.
