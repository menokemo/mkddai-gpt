...
## Decision: File/image understanding is two Tools, not a model change

باجوش currently only receives plain text — attached images and documents are silently dropped by the Pipe before reaching n8n. Considered switching باجوش's own model to a vision-capable one, or building a custom extraction pipeline inline — rejected both in favor of two separate `Call n8n Workflow Tool` sub-workflows: "Read Attached Document" (pure text extraction via n8n's built-in `Extract From File` node — no AI, no cost at all) and "Describe Attached Image" (calls a separate, deliberately cheap vision-capable model, only when an image is actually attached and باجوش decides to use the tool). This keeps باجوش's own model and per-turn cost completely unchanged — extra cost only occurs on the rare turns where a file/image is actually involved, same principle as the SearXNG search Tool. Deferred building this until the core New Project Path (Step 3b/4) is solid, since it's valuable but not blocking.
